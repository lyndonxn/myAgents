...
## 2026-09 升级（G1–G10，全部落地）

`feature/upgrade-2026-09` 分支完成的 10 个切片（详见 `HANDOFF.md`）：测试入口标准化（G1）→ 配置校验（G2）→ 数据外发默认关闭 + 回答状态披露（G3）→ 记忆治理（G4）→ 密钥安全 + 任务写回幂等（G5）→ 审计轨迹（G6）→ 任务看门狗（G7）→ 难评测集 + reward 模块（G8）→ 速度与成本包（G9）→ 检索迭代预算（G10）。全量 156 个离线测试。**延期挂账**：G11 本地 LLM 离线档位、付费 100 题实跑对比（ACC-U9-03/U10-03）、锁拆分与多 worker。
