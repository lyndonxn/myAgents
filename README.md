...
## 2026-09 升级（G1–G11，全部落地）

`feature/upgrade-2026-09` 分支完成的 11 个切片（详见 `HANDOFF.md`）：测试入口标准化（G1）→ 配置校验（G2）→ 数据外发默认关闭 + 回答状态披露（G3）→ 记忆治理（G4）→ 密钥安全 + 任务写回幂等（G5）→ 审计轨迹（G6）→ 任务看门狗（G7）→ 难评测集 + reward 模块（G8）→ 速度与成本包（G9）→ 检索迭代预算（G10）→ 本地 LLM 离线档位（G11，`llm.mode=local` + `scripts/prepare_offline.py --check`）。全量 173 个离线测试。**延期挂账**：付费 100 题实跑对比（ACC-U9-03/U10-03，需单独授权）、锁拆分与多 worker、coverage.py 覆盖率、P1-1/P1-4/P1-5。
...
- 系统默认本地监听，但模型回答仍会把问题发送到配置的外部服务；联网降级、长期记忆与实体记忆**默认关闭**，开启后数据/问题才外发或落盘（`data/`）。需要完全离线时保持默认并使用本地模型或仅运行检索评测。
