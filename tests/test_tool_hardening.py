...
def test_validate_required_missing():
    ...
    assert result.attempts == 1, f"校验失败不应重试，实际 attempts={result.attempts}"
    assert calls == [], "校验失败的步骤不应真正调用工具"

    # tools 层直接校验：缺 required 抛 ToolValidationError（ValueError 子类）
    try:
        validate_tool_input(kb, {})
        raise AssertionError("缺少 required 应抛 ToolValidationError")
...
def test_retry_then_success():
    ...
    assert result.output == {"text": "重试成功"}
    ...
    assert "RuntimeError" in result2.error and "一直失败" in result2.error

    print("✓ 失败重试（首次失败第二次成功 → attempts==2）")


def test_kb_fallback_to_web():
    ...
    assert result.ok, f"降级拿到可用结果后应 ok=True，实际 error={result.error}"
    assert result.degraded
    assert result.output["text"].startswith("[降级] 知识库检索失败，已改用 Web 搜索\n")
    assert "Web 搜索 RAG 是什么 的结果" in result.output["text"]  # 同一 query 传给了 web_search
    assert result.output["sources"] == ["https://example.com/a"]  # sources 正常收集

    print("✓ KB→Web 降级（degraded==True，text 含[降级]前缀）")


def test_kb_fallback_disabled_and_failed():
    ...
    assert "RuntimeError: KB 挂了" in result.error
    assert "[降级]" not in str(result.output)
    ...
    assert not result2.ok and not result2.degraded
    assert "KB 挂了" in result2.error and "Web 降级失败" in result2.error

    print("✓ 降级开关关闭 / 降级失败回退为普通失败")


def test_chat_json_repair_round():
    ...
    assert obj == {"ok": True}
    assert llm.calls == 2, f"应共 2 次模型调用，实际 {llm.calls}"

    # 修复轮消息 = 原 messages + 坏输出(assistant) + 修复指令(user)
    repair = llm.seen[1]
    assert [m["role"] for m in repair] == ["user", "assistant", "user"]
    assert repair[1]["content"] == "这不是 JSON"
    assert "无法解析为 JSON" in repair[2]["content"]
    assert "请只输出修正后的合法 JSON" in repair[2]["content"]

    print("✓ chat_json 修复轮（坏 JSON 一次修复成功，共 2 次调用）")


def test_chat_json_repair_exhausted():
    ...
    try:
        ...
        raise AssertionError("两次都坏应抛 LLMError")
    except LLMError as exc:
        assert llm.calls == 2, f"1 轮修复应共 2 次调用，实际 {llm.calls}"
        assert "无法从 LLM 输出解析 JSON" in str(exc)  # 保留原错误信息

    llm0 = ScriptedLLM(["坏输出"], Config({"llm": {"json_repair_rounds": 0}}))
    try:
        llm0.chat_json([{"role": "user", "content": "给我 JSON"}])
        raise AssertionError("rounds=0 应直接抛 LLMError")
    except LLMError:
        assert llm0.calls == 1
...
def test_validate_string_coercion_and_clamp():
    ...
    assert out["top_k"] == 5 and isinstance(out["top_k"], int)
    assert out["flag"] is True
    assert validate_tool_input(kb, {"query": "q", "flag": "false"})["flag"] is False

    # clamp：超出 min/max 的整数与数字字符串都收敛到边界
    assert validate_tool_input(kb, {"query": "q", "top_k": 99})["top_k"] == 10
    assert validate_tool_input(kb, {"query": "q", "top_k": "0"})["top_k"] == 1

    # 无法纠偏的字符串 → ToolValidationError
    try:
        validate_tool_input(kb, {"query": "q", "top_k": "abc"})
        raise AssertionError("'abc' 应纠偏失败")
    ...
    assert result.ok
    assert received == [{"query": "q", "top_k": 10, "flag": False}]
...
if __name__ == "__main__":
    ...
    test_kb_fallback_to_web()
    test_kb_fallback_disabled_and_failed()
    test_chat_json_repair_round()
    test_chat_json_repair_exhausted()
    test_validate_unknown_keys()
    test_validate_string_coercion_and_clamp()
    print("\nS1 工具层强化测试全部通过 ✅")
