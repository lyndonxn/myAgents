...
def test_acc_t4_03_sample_kb_and_questions_structure() -> None:
    docs = sorted(SAMPLES_KB.glob("*.md"))
    assert len(docs) == 5, f"样例文档应为 5 篇，实际 {[p.name for p in docs]}"
    stems: list[str] = []
    headings_by_stem: dict[str, list[str]] = {}
    for p in docs:
        text = p.read_text(encoding="utf-8")
        assert text.strip(), f"{p.name} 内容为空"
        h1, h2 = _doc_headings(text)
        assert len(h1) == 1, f"{p.name} 应恰好 1 个一级标题，实际 {len(h1)}"
        assert len(h2) >= 3, f"{p.name} 二级标题不足 3 个"
        stems.append(p.stem)
        headings_by_stem[p.stem] = h1 + h2

    questions = json.loads(QUESTIONS_SAMPLE.read_text(encoding="utf-8"))
    assert len(questions) == 10, f"样例题库应 10 题，实际 {len(questions)}"
    assert [q["id"] for q in questions] == [f"q{i:02d}" for i in range(1, 11)]

    for q in questions:
        for field in ("question", "expected_keywords", "expected_files", "expected_sections"):
            assert q.get(field), f"{q['id']} 缺字段 {field}"
        assert 3 <= len(q["expected_keywords"]) <= 5, f"{q['id']} 关键词数需 3~5"
        for ef in q["expected_files"]:
            assert any(ef in s for s in stems), f"{q['id']} expected_files 无法匹配样例文件名: {ef}"
        ...
        for es in q["expected_sections"]:
            assert any(es in h for h in headings_by_stem[target]), \
    ...
    for q in questions:
        if q["id"] in PARAPHRASE_IDS:
            ...
            assert not leaked, f"{q['id']} 问题泄漏标题词片段: {leaked}"
    print("✓ ACC-T4-03 样例库/题库结构校验（5 篇文档 + 10 题字段完整且绑定正确）")


def test_acc_t4_02_kb_override_only_builds_in_memory() -> None:
    ...
    try:
        ...
        assert build_calls == [False], f"--kb 应只以 persist=False 构建，实际 {build_calls}"
        assert load_calls == 0, "--kb 路径不得调用 load_index（读盘/可能重建写盘）"
        sample_questions = json.loads(QUESTIONS_SAMPLE.read_text(encoding="utf-8"))
        assert queries_seen == [q["question"] for q in sample_questions], \
            "--kb 运行应对样例题库逐题检索"

        # 对照：不带 --kb 保持原行为（load_index 加载既有索引，不构建）
        build_calls, load_calls = run_main(["run_benchmark.py", "--retrieval-only"])
        assert build_calls == [] and load_calls == 1, \
            f"默认路径应 load_index 且不构建，实际 build={build_calls} load={load_calls}"
...
def test_acc_t4_01_sample_kb_retrieval_without_touching_data() -> None:
    import benchmark.run_benchmark as rb
    ...
    assert config.kb_path == str(SAMPLES_KB.resolve())
    ...
    assert avg_file >= 0.8, f"平均文件命中 {avg_file:.0%} < 80%（ACC-T4-01 门槛）"
    assert _index_snapshot() == before, "运行后 data/ 用户索引文件被写入或改动"
    print("✓ ACC-T4-01 样例库检索全绿（文件命中 ≥80%）且 data/ 零污染")


if __name__ == "__main__":
    test_acc_t4_03_sample_kb_and_questions_structure()
    test_acc_t4_02_kb_override_only_builds_in_memory()
    test_acc_t4_01_sample_kb_retrieval_without_touching_data()
    print("\nT4 样例库评测测试全部通过 ✅")
