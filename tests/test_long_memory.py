...
def test_acc_s4_01_semantic_search():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert hits[0].id == "e1", f"top1 应为语义最近条目 e1，实际 {hits[0].id}"
        assert hits[0].hits == 1, "命中者 hits 应 +1"

        # 换主题同样命中
        hits2 = ltm.search("午餐 吃什么", k=1)
        assert hits2[0].id == "e3"
...
def test_acc_s4_02_prune_semantics():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert [e.id for e in ltm.episodes] == ["e1", "e4", "e5"], \
            f"应保留 hits 高的 e1 与最新的 e4/e5，实际 {[e.id for e in ltm.episodes]}"
        assert {e.id for e in evicted} == {"e2", "e3"}, "被淘汰的应是 hits 低且最旧的 e2/e3"
        # 淘汰后 TF-IDF 整体重建（re-fit），检索仍正常
        assert ltm.search("主题5", k=1)[0].id == "e5"
...
def test_exclude_and_as_context():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert "e2" not in ids and "e1" in ids and "e3" in ids, f"排除最近 1 条语义不符: {ids}"
        # (sA, 2)：排除 sA 全部
        ids2 = [h.id for h in ltm.search("苹果", k=5, exclude_session_recent=("sA", 2))]
        assert "e1" not in ids2 and "e2" not in ids2 and "e3" in ids2
        # exclude_session：排除该会话全部 episode
        hits3 = ltm.search("苹果", k=5, exclude_session="sA")
        assert all(h.session_id != "sA" for h in hits3) and [h.id for h in hits3] == ["e3"]

        block = ltm.as_context("苹果 发布会", k=2, exclude_session_recent=("sA", 1))
        assert block.startswith("相关历史经验：")
        assert "[1] Q: 苹果公司股价" in block and "A: 涨了" in block

        # 空库 / 全部被排除 → 空串
        empty = LongTermMemory(Path(td) / "none", TfidfHashEmbeddingBackend(hash_dim=64))
        assert empty.as_context("任何问题", k=3) == ""
        assert ltm.as_context("苹果", k=3, exclude_session="sA", exclude_session_recent=("sB", 1)) == ""
...
def test_persistence_roundtrip_and_backend_mismatch():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert (store / "episodes.json").exists() and (store / "vectors.npy").exists() and (store / "meta.json").exists()
        assert not list(store.glob("*.tmp")), "原子替换后不应残留 tmp 文件"

        ltm2 = LongTermMemory(store, TfidfHashEmbeddingBackend(hash_dim=128), max_episodes=10)
        assert [e.id for e in ltm2.episodes] == ["e1", "e2"], "重载后条目一致"
        assert ltm2.episodes[0].hits == 1, "hits 计数应随 episodes.json 持久化"
        assert ltm2.search("RAG 流程", k=1)[0].id == "e1", "重载后检索仍可用"

        # backend 名不一致 → 向量不可比 → 重建空库
        ltm3 = LongTermMemory(store, _OtherBackend(), max_episodes=10)
        assert ltm3.episodes == []

        # episodes.json 损坏 → 空库不崩溃
        bad = Path(td) / "bad"
        bad.mkdir()
        (bad / "episodes.json").write_text("{{{不是JSON", encoding="utf-8")
        assert LongTermMemory(bad, TfidfHashEmbeddingBackend(hash_dim=64)).episodes == []

    print("✓ 原子持久化 roundtrip（无 tmp 残留 / hits 保留 / backend 不一致与损坏文件 → 空库）")


def test_non_tfidf_backend_alignment():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert [e.id for e in ltm.episodes] == ["e3", "e4", "e5"], "超容量自动淘汰最旧两条"
        assert ltm._vectors.shape[0] == ltm.size, "向量行数应与条目数对齐"
        assert ltm.search("主题5", k=1)[0].id == "e5", "淘汰后检索仍对齐"

        ltm2 = LongTermMemory(store, _HashBackend(), max_episodes=3)
        assert [e.id for e in ltm2.episodes] == ["e3", "e4", "e5"]
        assert ltm2.search("主题5", k=1)[0].id == "e5"
...
def test_memory_compressor_normal_and_fallback():
    ...
    assert out == "合并后的摘要" and len(out) <= 300
    user = llm.seen[0][1]["content"]
    assert "旧摘要内容" in user and "什么是 RAG？" in user and "MCP 呢？" in user, "prompt 应携带旧摘要与各轮问答"
    ...
    assert out2.startswith("旧摘要。"), "降级拼接应包含旧摘要"
    assert "问：什么是 RAG／答：RAG 是检索增强生成" in out2, "各轮取首句"
    assert "它先检索再生成" not in out2, "首句后的内容不进入降级摘要"
    assert len(out2) <= 600

    # LLM 输出超长 → 截断到 300
    assert len(MemoryCompressor(ScriptedLLM(["长" * 400], cfg)).compress("", turns)) == 300

    print("✓ MemoryCompressor 正常/抛错两路（≤300 摘要 / ≤600 降级拼接）")


def test_acc_s4_03_session_memory_compress():
    ...
    assert summary == "这是压缩后的摘要" and mem.summary == summary
    assert [t.question for t in mem.turns] == ["q2", "q3"], "窗口内轮次保留"
    text = mem.as_text()
    assert text.startswith("此前对话摘要：这是压缩后的摘要"), f"as_text 应含摘要块: {text!r}"
    assert "用户：q2" in text and "用户：q3" in text

    # 再次溢出：旧摘要作为输入传给压缩器
    mem.add("q4", "a4", [], "")  # q2 滑出
    llm2 = ScriptedLLM(["第二次摘要"], cfg)
    mem.maybe_compress(llm2)
    assert "这是压缩后的摘要" in llm2.seen[0][1]["content"], "旧摘要应传入压缩 prompt"
    assert mem.summary == "第二次摘要"
    ...
    assert "问题甲" in s and "答案甲" in s, f"降级拼接应含滑出轮首句: {s!r}"
    assert len(s) <= 600
    assert mem3.as_text().startswith("此前对话摘要：")
...
def test_acc_s4_04_entity_extract_robustness():
    """ACC-S4-04：实体抽取 LLM 抛错/返回坏 JSON → 返回 {} 不崩溃；正常抽取含过滤。"""
    cfg = Config({})
    assert EntityMemory().extract(ScriptedLLM(["not json", "still not json"], cfg), "q", "a") == {}
    assert EntityMemory().extract(ScriptedLLM(['{"foo": 1}'], cfg), "q", "a") == {}          # 缺 entities
    assert EntityMemory().extract(ScriptedLLM(['{"entities": [1, 2]}'], cfg), "q", "a") == {}  # entities 非 dict
    ...
    assert EntityMemory().extract(_BoomLLM(cfg), "q", "a") == {}
    ...
    assert out == {"RAG": "检索增强生成", "MCP": "模型上下文协议"}, f"超长名/空名/空事实应丢弃: {out}"

    payload6 = json.dumps({"entities": {f"实{i}": "事实" for i in range(7)}}, ensure_ascii=False)
    assert len(EntityMemory().extract(ScriptedLLM([payload6], cfg), "q", "a")) == 5, "实体数上限 5"

    print("✓ ACC-S4-04 实体抽取 抛错/坏 JSON → {}；正常抽取过滤超长与空值、上限 5")


def test_entity_memory_merge_prompt_save_load():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert len(em.entities["MCP"]["facts"]) == 1
        em.entities["RAG"]["last_seen"] = "2026-08-01T10:00:00"
        em.entities["MCP"]["last_seen"] = "2026-08-02T10:00:00"

        prompt = em.as_prompt()
        assert prompt.startswith("已知实体：")
        assert prompt.index("MCP") < prompt.index("RAG"), "应按 last_seen 倒序"
        assert "（最近提及 2026-08-02T10:00:00）" in prompt

        em.merge({f"n{i}": "事实" for i in range(10)})
        assert len(em.as_prompt(limit=3).splitlines()) == 4, "标题 + limit 行"

        em.save()
        em2 = EntityMemory(store)
        em2.load()
        assert em2.entities == em.entities, "save/load roundtrip 一致"
...
def test_config_memory_defaults():
    """memory.* 四个配置项的默认值与覆盖。"""
    cfg = Config({})
    assert cfg.memory_long_term_enabled is True
    assert cfg.memory_entities_enabled is True
    assert cfg.memory_max_episodes == 200
    assert cfg.memory_embedding_backend == "auto"
    cfg2 = Config({"memory": {"long_term_enabled": False, "entities_enabled": False,
                              "max_episodes": 50, "embedding_backend": "tfidf"}})
    assert cfg2.memory_long_term_enabled is False
    assert cfg2.memory_entities_enabled is False
    assert cfg2.memory_max_episodes == 50
    assert cfg2.memory_embedding_backend == "tfidf"

    print("✓ config memory.* 默认开、默认 200 条、默认 auto，可被 config.yaml 覆盖")


def test_planner_prompt_longterm_injection():
    """build_planner_prompt 注入 longterm_text 块；为空时不注入。"""
    msgs = build_planner_prompt("问题", ["工具A"], 5, "历史", "相关历史经验：\n[1] Q: x\n    A: y")
    user = msgs[1]["content"]
    assert "相关历史经验：" in user and "[1] Q: x" in user
    assert user.index("对话历史") < user.index("相关历史经验") < user.index("可用工具")
    msgs2 = build_planner_prompt("问题", ["工具A"], 5)
    assert "相关历史经验" not in msgs2[1]["content"]
...
def test_agent_ask_long_term_integration():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert a1.ok, f"第一次 ask 不应失败: {a1.error}"
        lm = agent.long_memory
        assert lm is not None and lm.size == 1, "成功回答应写入 1 条 episode"
        ep = lm.episodes[0]
        assert ep.session_id == "s1" and ep.question == "什么是 RAG"
        assert ep.answer_summary.startswith("最终答案")
        assert ep.entities == {"RAG": "检索增强生成"} and ep.sources == ["kb://a.md"]
        assert (Path(td) / "memory" / "episodes.json").exists()
        assert (Path(td) / "memory" / "entities.json").exists()
        assert agent.entity_memory.entities["RAG"]["facts"] == ["检索增强生成"]

        a2 = agent.ask("再讲讲 RAG", session_id="s2")
        assert a2.ok and a2.final_answer == "第二次回答[1]"
        plan_user = llm.seen[5][1]["content"]  # 第二次 ask 的规划 prompt（0规划 1反思 2合成 3实体 4改写 5规划）
        assert "相关历史经验：" in plan_user and "Q: 什么是 RAG" in plan_user, \
            f"规划 prompt 应注入召回的历史经验: {plan_user[:200]}"
        assert lm.size == 2
        assert lm.episodes[0].hits == 1, "第一次召回命中应使 hits+1 并持久化"
        assert llm.calls == 9, f"两次 ask 共 9 次 LLM 调用（含实体抽取），实际 {llm.calls}"

    print("✓ Agent.ask 集成：episode/实体写入与持久化、规划 prompt 收到召回历史经验、hits 计数")


def test_agent_ask_failure_no_episode():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert not a.ok
        assert agent.long_memory.size == 0
        assert agent.memory.count == 0
        assert not (Path(td) / "memory" / "episodes.json").exists()
        assert not (Path(td) / "memory" / "entities.json").exists()

    print("✓ 失败回答不写长期记忆/实体/会话记忆")


def test_agent_switches_disabled():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert agent.long_memory is None and agent.entity_memory is None

        a = agent.ask("问题", session_id="s1")
        assert a.ok and agent.memory.count == 1
        assert llm.calls == 3, f"关闭记忆增强后只有规划/反思/合成 3 次调用，实际 {llm.calls}"
        assert not (Path(td) / "memory").exists()

    print("✓ memory.* 开关关闭：不建库、无额外 LLM 调用、无落盘")


def test_agent_no_session_no_side_effects():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        a = agent.ask("问题")
        assert a.ok
        assert llm.calls == 3, f"无会话时不应有实体抽取调用，实际 {llm.calls}"
        assert not (Path(td) / "memory").exists()
        assert "相关历史经验" not in llm.seen[0][1]["content"]
        assert agent.long_memory.size == 0  # 惰性建库为空，且未产生落盘
        assert not (Path(td) / "memory" / "episodes.json").exists()
...
if __name__ == "__main__":
    test_acc_s4_01_semantic_search()
    test_acc_s4_02_prune_semantics()
    test_exclude_and_as_context()
    test_persistence_roundtrip_and_backend_mismatch()
    test_non_tfidf_backend_alignment()
    test_memory_compressor_normal_and_fallback()
    test_acc_s4_03_session_memory_compress()
    test_acc_s4_04_entity_extract_robustness()
    test_entity_memory_merge_prompt_save_load()
    test_config_memory_defaults()
    test_planner_prompt_longterm_injection()
    test_agent_ask_long_term_integration()
    test_agent_ask_failure_no_episode()
    test_agent_switches_disabled()
    test_agent_no_session_no_side_effects()
    print("\nS4 长期记忆与摘要压缩/实体记忆测试全部通过 ✅")
