...
def test_placeholder_field_and_full():
    ...
    assert received[0] == {"q": "第一步"}
    assert received[1] == {"q": "X", "full": "步骤一全文", "mixed": "前缀X后缀", "top_k": 3}, \
        f"占位符解析结果不符: {received[1]}"
    assert isinstance(received[1]["top_k"], int), "解析出的 '3' 应经 schema 纠偏为整数（解析先于校验）"
    # 只在执行时替换：Plan 的 step.input 保持占位符原样
    assert plan.steps[1].input["q"] == "@step:1.query"
    assert plan.steps[1].input["full"] == "@step:1"

    print("✓ ACC-S2-01 占位符解析（field/全文/子串替换/解析先于校验，Plan 不被改动）")


def test_placeholder_upstream_missing_or_failed():
    ...
    assert results[1].ok and received[-2] == {"q": ""}, "上游失败的引用应置空串且不崩溃"
    assert results[2].ok and received[-1] == {"q": ""}, "缺失步骤的引用应置空串且不崩溃"
    joined = "\n".join(handler.messages)
    assert "@step:1.text" in joined and "未成功" in joined, f"应记录上游失败的 warning: {joined}"
    assert "@step:9.query" in joined and "缺失" in joined, f"应记录缺失步骤的 warning: {joined}"

    print("✓ ACC-S2-01 上游缺失/失败置空并记录 warning（不崩溃）")


def test_placeholder_non_dict_output():
    ...
    assert results[2].ok
    assert received[0] == {"a": "纯文本输出", "b": ""}, f"非 dict 输出语义不符: {received[0]}"
    ...
    assert received[0]["a"] == str({"query": "X"}), f"dict 无 text 应 str(output): {received[0]['a']}"
    assert received[0]["b"] == ""
...
def test_reflection_on_failure_appends_steps():
    ...
    assert answer.ok, f"补步成功后 ask 不应失败: {answer.error}"
    assert answer.final_answer == "最终答案"
    assert answer.plan.rounds == 2, f"补步后 rounds 应为 2，实际 {answer.plan.rounds}"
    assert answer.plan.reflections == ["步骤1失败，需要用步骤2的主题补充检索"]
    assert [s.step_id for s in answer.plan.steps] == [1, 2, 3], "补步 step_id 应续号"
    assert len(answer.steps) == 3 and answer.steps[2].ok, "补步应被执行且成功"
    assert kb_calls == ["X 补充"], f"补步应执行且 @step:2.topic 解析为首轮输出字段: {kb_calls}"
    # 反思 prompt 携带轨迹（含失败步骤 ok=false 与输出摘要）
    reflect_user = llm.seen[1][1]["content"]
    assert '"ok": false' in reflect_user and "flaky" in reflect_user
...
def test_hard_cap_truncates_and_stops_reflection():
    ...
    assert [s.step_id for s in answer.plan.steps] == [1, 2, 3, 4], "应截断到硬上限 4 步并续号"
    assert len(probe_calls) == 4 and probe_calls[-1] == {"q": "补1-1"}, f"只执行截断后的补步: {probe_calls}"
    assert answer.plan.rounds == 2
    assert len(answer.plan.reflections) == 1
    assert llm.calls == 3, f"达到硬上限后不应再反思（规划+反思+合成=3），实际 {llm.calls}"
    assert answer.final_answer == "答案"
...
def test_bad_reflection_json_is_silent():
    ...
    assert answer.ok, f"反思失败不应影响回答: {answer.error}"
    assert answer.final_answer == "最终答案"
    assert len(answer.plan.steps) == 1 and len(probe_calls) == 1, "不应有补步"
    assert answer.plan.rounds == 1 and answer.plan.reflections == []
    assert llm.calls == 4, f"规划1+反思2（含修复轮）+合成1=4，实际 {llm.calls}"
...
def test_reflect_disabled_skips_reflection():
    ...
    assert answer.ok  # 步骤失败不等于问答失败
    assert answer.plan.rounds == 1 and answer.plan.reflections == []
    assert len(answer.plan.steps) == 1
    assert llm.calls == 2, f"关闭反思时只有规划+合成两次调用，实际 {llm.calls}"

    print("✓ reflect 开关关闭时不反思（即使有失败步骤）")


def test_max_reflections_zero_skips_reflection():
    ...
    assert answer.plan.rounds == 1 and answer.plan.reflections == []
    assert len(answer.plan.steps) == 1
    assert llm.calls == 2, f"max_reflections=0 时不应反思，实际 {llm.calls}"
...
def test_planner_reflect_direct():
    ...
    assert [s.step_id for s in plan3.steps] == [1, 2]

    print("✓ Planner.reflect 直测（need_more=false / 坏结构抛 LLMError / 预算截断）")


if __name__ == "__main__":
    test_placeholder_field_and_full()
    test_placeholder_upstream_missing_or_failed()
    test_placeholder_non_dict_output()
    test_reflection_on_failure_appends_steps()
    test_hard_cap_truncates_and_stops_reflection()
    test_bad_reflection_json_is_silent()
    test_reflect_disabled_skips_reflection()
    test_max_reflections_zero_skips_reflection()
    test_planner_reflect_direct()
    print("\nS2 ReAct 迭代与反思重规划测试全部通过 ✅")
