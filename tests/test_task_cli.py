...
def test_acc_t3_01_list():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert code == 0
        assert tid_done in out and tid_run in out, f"应列出任务 id: {out}"
        assert "已完成" in out and "执行中" in out, f"应显示中文状态: {out}"
        assert long_q[:24] in out and "…" in out, "超过 24 字的问题应截断并加省略号"
        assert long_q not in out, "列表不应打印问题全文"
        assert out.index(tid_run) < out.index(tid_done), "应按 updated_at 倒序"
...
def test_acc_t3_02_question_flow():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            release.join()
        assert code == 0, f"应正常完成: {out}"
        assert "任务已创建" in out
        assert "执行中" in out, f"轮询应观察到 running 状态: {out}"
        assert "已完成" in out
        assert "✓" in out and "probe" in out, f"应打印 ✓ 步骤行: {out}"
        assert "这是最终答案[1][2]" in out, "应打印最终答案全文"
        assert "参考来源" in out and "kb://q1.md" in out and "kb://q2.md" in out
        assert "tokens in=" in out and "估算成本" in out, "应打印 token/成本 usage"

        # 落盘结果与输出一致
        records = TaskStore(store_dir).list()
        assert len(records) == 1
        rec = records[0]
        assert rec.status == "completed" and rec.question == "两步任务"
        assert rec.final_answer == "这是最终答案[1][2]"
        assert rec.sources == ["kb://q1.md", "kb://q2.md"]
        assert "prompt_tokens" in rec.usage and "completion_tokens" in rec.usage and "cost_yuan" in rec.usage
...
def test_acc_t3_03_missing_id_and_bad_resume():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for func in (task_mod.run_watch, task_mod.run_resume, task_mod.run_cancel):
            ...
            assert code == 1, f"{func.__name__} 对不存在 id 应返回 1"
            assert f"任务不存在: {missing}" in out, f"应打印友好错误: {out}"

        # 非法 id（含路径穿越）同样视为不存在
        code, out, _ = _capture(task_mod.run_watch, "../evil", store_dir=store_dir)
        assert code == 1 and "任务不存在" in out
        ...
        assert code == 1
        assert "已完成" in out, f"原因应包含当前状态: {out}"
        assert "paused" in out and "恢复" in out
...
def test_cancel_existing_task():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert code == 0
        assert store.get(tid).status == "canceled", "应经 runner.cancel 落盘 canceled 终态"
        assert "已取消" in out, f"应打印终态: {out}"

        # 已是终态：提示无需取消并打印终态，退出 0
        code, out, _ = _capture(task_mod.run_cancel, tid, store_dir=store_dir, runner=runner)
        assert code == 0 and "无需取消" in out and "已取消" in out
...
def test_resume_paused_task_flow():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert store.recover_running() == [tid]
        ...
        assert code == 0, f"恢复后应完成: {out}"
        assert "已恢复执行" in out and "已完成" in out
        assert "恢复后的答案[1]" in out
        rec = store.get(tid)
        assert rec.status == "completed" and rec.final_answer == "恢复后的答案[1]"
        assert len(rec.steps) == 1 and rec.steps[0]["ok"], "持久化步骤应被执行并落盘"
        ...
        try:
            with contextlib.redirect_stdout(out_buf):
                try:
                    ...
                    raise AssertionError("无 Key 时应报错退出")
                except SystemExit as exc:
                    assert exc.code == 1, f"退出码应为 1: {exc.code}"
        finally:
            if saved is not None:
                os.environ["DEEPSEEK_API_KEY"] = saved
        out = out_buf.getvalue()
        assert "错误" in out and "API Key" in out, f"应打印中文错误提示: {out}"
        assert not list(Path(td).iterdir()), "失败发生在装配期，不应产生任何存储副作用"
...
def test_keyboard_interrupt_background_message():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert code == 0, f"Ctrl+C 应返回 0: {out}"
        assert "任务已创建" in out
        assert "任务转入后台，可稍后 --watch 查看" in out, f"应打印转后台提示: {out}"

        # 清理：放行被门控阻塞的工作线程，等待任务自然到终态（不遗留阻塞线程）
        gate.set()
        store = TaskStore(store_dir)
        tid = store.list()[0].task_id
        assert _wait_until(lambda: (store.get(tid) or TaskRecord()).status in TERMINAL_STATUSES)
...
def test_watch_timeout_returns_nonzero():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert code == 1, f"未到终态超时应返回 1: {out}"
        assert "等待超时" in out and tid in out
        assert store.get(tid).status == "running", "超时观察不应打断任务执行"
...
def test_main_argparse_dispatch():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            with contextlib.redirect_stdout(out_buf):
                try:
                    task_mod.main()
                    raise AssertionError("无参数应退出 1")
                except SystemExit as exc:
                    assert exc.code == 1
            assert "usage" in out_buf.getvalue().lower(), "应打印用法"
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                try:
                    task_mod.main()
                    raise AssertionError("互斥参数应退出 1")
                except SystemExit as exc:
                    assert exc.code == 1
            ...
            with contextlib.redirect_stdout(out_buf):
                try:
                    task_mod.main()
                    raise AssertionError("--list 应退出 0")
                except SystemExit as exc:
                    assert exc.code == 0
            assert "暂无任务" in out_buf.getvalue()
        finally:
            sys.argv = saved_argv
...
if __name__ == "__main__":
    test_acc_t3_01_list()
    test_acc_t3_02_question_flow()
    test_acc_t3_03_missing_id_and_bad_resume()
    test_cancel_existing_task()
    test_resume_paused_task_flow()
    test_no_api_key_exit()
    test_keyboard_interrupt_background_message()
    test_watch_timeout_returns_nonzero()
    test_main_argparse_dispatch()
    print("\nT3 CLI 任务模式测试全部通过 ✅")
